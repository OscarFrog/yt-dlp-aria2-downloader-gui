...
volontairement traités par le téléchargeur natif de yt-dlp. La version actuelle est la **2.2.1**.
...
./yt-dlp-aria2-downloader-gui_2.2.1-1_all.deb`. Pour **les autres distributions
...
gh release verify-asset v2.2.1 ./ARTEFACT -R OscarFrog/yt-dlp-aria2-downloader-gui
```
...
```bash
gh workflow run release.yml   --ref v2.2.1   -f tag=v2.2.1   -R OscarFrog/yt-dlp-aria2-downloader-gui
```
...
La release 2.2.1 publie un DEB indépendant de l'architecture, aligné sur le

...
volontairement traités par le téléchargeur natif de yt-dlp. La version actuelle est la **2.2.6**.
...
./yt-dlp-aria2-downloader-gui_2.2.6-1_all.deb`. Pour **les autres distributions
...
gh release verify-asset v2.2.6 ./ARTEFACT -R OscarFrog/yt-dlp-aria2-downloader-gui
```
...
```bash
gh workflow run release.yml   --ref v2.2.6   -f tag=v2.2.6   -R OscarFrog/yt-dlp-aria2-downloader-gui
```
...
La release 2.2.6 publie un DEB indépendant de l'architecture, aligné sur le

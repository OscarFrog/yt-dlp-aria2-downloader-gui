...
téléchargeur natif de yt-dlp. La version actuelle est la **2.1.32**.
...
yt-dlp-aria2-downloader-gui-2.1.32-1.fc44.noarch.rpm
SHA256SUMS
```
...
bash ./install-fedora.sh ./yt-dlp-aria2-downloader-gui-2.1.32-1.fc44.noarch.rpm
```
...
./yt-dlp-aria2-downloader-gui_2.1.32-1_all.deb`. Pour **les autres distributions
...
gh release verify v2.1.32 -R OscarFrog/yt-dlp-aria2-downloader-gui
gh release verify-asset v2.1.32 ./ARTEFACT -R OscarFrog/yt-dlp-aria2-downloader-gui
```
...
```bash
gh workflow run release.yml   --ref v2.1.32   -f tag=v2.1.32   -R OscarFrog/yt-dlp-aria2-downloader-gui
```
...
La release 2.1.32 publie un DEB indépendant de l'architecture, aligné sur le
même modèle de runtimes gérés que Fedora. Téléchargez :

```text
yt-dlp-aria2-downloader-gui_2.1.32-1_all.deb
SHA256SUMS
```
...
```bash
sha256sum --ignore-missing --check SHA256SUMS
sudo apt install ./yt-dlp-aria2-downloader-gui_2.1.32-1_all.deb
```
...
```text
yt-dlp-aria2-downloader-gui-2.1.32.zip
SHA256SUMS
```
...
```bash
sha256sum --ignore-missing --check SHA256SUMS
unzip yt-dlp-aria2-downloader-gui-2.1.32.zip
cd yt-dlp-aria2-downloader-gui-2.1.32

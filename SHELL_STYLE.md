...
1. a read-only preparation job binds the unchanged source version and the
   authenticated main/target/tag reference snapshot before candidate execution; the
   candidate runs without network or publication authority and emits only a
   data-only patch and digest;
2. a fresh read-only verifier checks the immutable clean base, handoff digest,
   strict shell/pin allowlist, upstream provenance, unchanged source version and
   canonical equivalence under the previously trusted formatter against that
   base, then runs the complete project validation in a separate container with no

...
| `CI_AUDIT.md` | Audit report | Source-only dated CI/runtime/test timing, duplication, protection and implementation evidence, including Phase 2 before/after benchmarks and adversarial review; records limitations without replacing operating policy. | KEEP |
...
| `tests/lib/test-runner.sh` | Sourced Bash/Python test library | Source-only bounded child/session supervision, same-PID monitor-group transition, authenticated session identity, cancellation and timing helpers used by the canonical and repetition runners; preserves caller options. | KEEP |
...
| `tests/test-runner-integration.sh` | Integration test | Source-only fast/full regression for runner process supervision, monitor-group/session transitions, scheduling, status propagation and exact validation manifests. | KEEP |

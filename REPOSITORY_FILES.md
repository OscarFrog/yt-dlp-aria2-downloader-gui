...
| `scripts/git-inspect.sh` | Bash développement | Exécute sous la politique sandbox/baseline un ensemble fermé de consultations Git sans argument libre, pager, filtre ni diff externe | Agents Codex, mainteneurs, `.codex/rules/default.rules`, `test-static.sh` | Oui — inspection contributive sûre, présente dans le ZIP source mais non installée par RPM/DEB | Oui — actions status/diff/diff-staged/diff-check/inventory fixes, environnement fermé, Git système et écritures optionnelles désactivées | Non | Non, fournit la frontière exécutable que les règles par préfixe ne peuvent pas exprimer seules | KEEP |

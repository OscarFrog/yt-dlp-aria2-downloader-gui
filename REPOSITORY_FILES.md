...
| `.github/workflows/packages.yml` | CI workflow | Source-only package CI: validates a Git-free archive, builds RPM/DEB, authenticates the previous immutable release and exercises package lifecycle/upgrades. | KEEP |
| `.github/workflows/qualification.yml` | CI workflow | Source-only supported FFmpeg generation matrix; the Fedora download job uses disposable local storage volumes subject to private-filesystem validation. | KEEP |
| `.github/workflows/real-tools.yml` | CI workflow | Source-only qualification with pinned real yt-dlp/aria2/FFmpeg fixtures and repeated boundary tests; separately checks scheduled current-stable yt-dlp. | KEEP |
...
| `.github/workflows/release.yml` | CI workflow | Source-only authorized tag pipeline: validates source, builds exact artifacts, isolates RPM signing, tests upgrades, attests immutable publication and verifies public downloads. | KEEP |
| `.github/workflows/shell.yml` | CI workflow | Source-only canonical Ubuntu/Fedora validation, early pinned actionlint and actual Python 3.10 qualification; the Fedora job gives private-state and media tests disposable local storage volumes. | KEEP |
...
| `.github/workflows/stress.yml` | CI workflow | Source-only repeated mock cancellation, runtime hardening and package-cleanup qualification; complements a single canonical full run. | KEEP |
...
| `scripts/release-evidence-qualification.sh` | Release qualification | Source-only maintainer helper; reads public release/assets, attestations and exact-SHA/scheduled Actions evidence, then writes a local qualification report. | KEEP |
...
| `tests/aria2-real-behavior-integration.sh` | Real-tool qualification | Source-only direct-transfer Range/no-Range/redirect/error/cancel/restart tests against a controlled server; run separately by real-tool/release CI. | KEEP |
...
| `tests/ffmpeg-generation-qualification.sh` | Qualification wrapper | Source-only entrypoint for the FFmpeg generation matrix; verifies expected tool versions and reuses real-tool/progress/duration compatibility tests. | KEEP |
...
| `tests/mock-integration.sh` | Integration test | Source-only grouped engine/GUI tests including permissive destination simulation, sentinel secrecy, signal cancellation, old-residue preservation and unconfirmed-shutdown cleanup. | KEEP |
| `tests/package-user-cleanup-integration.sh` | Integration test | Source-only fast/full and stress regression for legacy launchers, custom-XDG ownership records, symlinks and allowlisted RPM user cleanup. | KEEP |
...
| `tests/repeat-qualification.sh` | Bash qualification runner | Source-only bounded parallel repetition of independent commands with ordered logs; shared by stress, package, real-tool and release qualification. | KEEP |
...
- Tracked and new source files included: **94**.
- Inventory entries: **94**.
- Files retained: **94**.

...
| `.codex/rules/default.rules` | Politique Codex mécanique | Demande confirmation avant les mutations Git/GitHub distantes et interdit les formes usuelles de force-push vers `main` | Codex dans un projet approuvé, `test-static.sh`, mainteneurs | Oui — garde-fou contributif présent dans le ZIP source mais non installé par RPM/DEB | Oui — règles conservatrices sans décision `allow`, exemples positifs/négatifs validés | Non | Non, rend mécaniques certaines limites d’`AGENTS.md` sans accorder d’autorité | KEEP |
...
| `.github/pull_request_template.md` | Modèle GitHub | Exige résumé, risques, validations réelles et contrôles de cohérence avant revue | Contributeurs, GitHub Pull Requests, `test-static.sh` | Oui — revue et livraison, présent dans le ZIP source mais non installé par RPM/DEB | Oui — distingue commandes exécutées, limites et mutations externes | Non | Non, transforme la Definition of Done en preuve par PR | KEEP |
...
| `test-static.sh` | Bash test | Valide inventaires, en-têtes, skills/règles/modèles Codex, workflows, versions, packaging et contrats source | `run-all.sh`, workflows, mainteneurs | Oui — CI statique | Oui — couvre Python, outils contributifs et inventaire de tous les fichiers | Non | Non | KEEP |
...
| `tests/test-runner-integration.sh` | Bash test | Teste doctor JSON, concurrence, timing, transitions finales et terminaison du runner | `run-all.sh`, tests statiques | Oui — infrastructure tests | Oui — diagnostic hermétique et stress de signaux inclus | Non | Non | KEEP |

...
| `.github/workflows/release-docs.yml` | GitHub Actions | Prépare, revérifie et propose par PR l’alignement des références documentaires après une release immuable réussie | `workflow_run` de `release.yml`, helper de version publiée, GitHub PR | Oui — empêche la dérive des commandes d’installation après publication | Oui — permissions minimales par job, handoff data-only vérifié et publisher sans exécution du code du dépôt | Non | Non, complète la publication sans modifier directement `main` | KEEP |

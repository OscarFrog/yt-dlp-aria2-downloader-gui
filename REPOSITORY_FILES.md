...
La première colonne de la table ci-dessous contient exactement les **86**
...
| `.codex/rules/default.rules` | Politique Codex mécanique | Route l’inspection Git non interactive vers un helper fermé sans lui accorder d’autorisation explicite, demande confirmation pour tout Git direct, GitHub CLI, options Git globales et wrappers d’environnement hors sandbox, et interdit les formes usuelles de force-push vers `main` | Codex dans un projet approuvé, `test-static.sh`, mainteneurs | Oui — garde-fou contributif présent dans le ZIP source mais non installé par RPM/DEB | Oui — aucune règle `allow`, le helper reste soumis à la politique sandbox/baseline, les mutations directes restent interactives et les refspecs `main` courantes sont couvertes | Non | Non, rend mécaniques certaines limites d’`AGENTS.md` sans accorder d’autorité de mutation | KEEP |
...
| `.github/workflows/release-docs.yml` | GitHub Actions | Prépare, revérifie et publie une branche bornée pour l’alignement documentaire après une release immuable réussie | `workflow_run` de `release.yml`, helper de version publiée, PR ouverte par le mainteneur | Oui — empêche la dérive des commandes d’installation après publication | Oui — permissions minimales par job, handoff data-only vérifié, aucun checkout privilégié ni permission pull request | Non | Non, complète la publication sans modifier directement `main` | KEEP |
...
| `.github/workflows/shfmt-update.yml` | GitHub Actions | Prépare, vérifie et publie une branche de mise à jour du pin shfmt | Planification, dispatch, pin shfmt, PR ouverte par le mainteneur | Oui — maintenance formatter/supply chain | Oui — candidat isolé pendant formatage et validation, source read-only et publisher data-only sans permission pull request | Non | Non | KEEP |
...
| `CHANGELOG.md` | Documentation historique | Journal chronologique des releases et décisions | Utilisateurs, packaging, release, audit de versions | Oui — documentation/release | Oui — version courante et entrées anciennes figées | Oui — valeur historique explicite | Non, ne remplace pas les docs courantes | KEEP — HISTORIQUE |
| `LICENSE` | Licence | Texte MIT du projet | Utilisateurs, README, RPM | Oui — juridique | Oui — texte MIT complet | Non | Non | KEEP |
| `README.fr.md` | Documentation utilisateur | Guide utilisateur français complet | Utilisateurs, packages, ZIP, release | Oui — documentation installée | Oui — parité fonctionnelle et versions de développement/publiée vérifiées | Compatibilité cleanup documentée et justifiée | Non, traduction maintenue | KEEP |
...
| `download-video.sh` | Bash production | Moteur CLI pour une URL, média et publication finale | GUI, launchers, packages, tests réels/mocks | Oui — cœur d’exécution, installé | Oui — version et interface/manpage alignées | Récupération de staging ancien volontaire | Non | KEEP |
...
| `packaging/package-user-cleanup.sh` | Bash migration/cleanup | Retire à l’installation RPM uniquement les schémas desktop portables historiques reconnus exactement, puis supprime les données allowlistées lors de l’effacement final | Scriptlets spec RPM, install-tree, tests cleanup | Oui — migration et désinstallation RPM, installée seulement en RPM | Oui — lignes fixes, cible directe canonique sous HOME, unlink non récursif, bornes HOME/XDG/symlink et modes best-effort explicites | Oui — variantes de launchers génériques et chemins GUI historiques encore nécessaires | Non | KEEP — COMPATIBILITÉ |
...
| `packaging/rpm/yt-dlp-aria2-downloader-gui.spec` | Spec RPM | Déclare dépendances, installation, `%files`, scriptlets de migration/cleanup et changelog RPM | rpmbuild via `build-rpm.sh` | Oui — packaging RPM | Oui — version macro, payload, migration launcher bornée et changelog figé cohérents | Changelog embarqué légitime | Non | KEEP |
...
| `scripts/git-inspect.sh` | Bash développement | Exécute sous la politique sandbox/baseline un ensemble fermé de consultations Git sans argument libre, pager, filtre ni diff externe | Agents Codex, mainteneurs, `.codex/rules/default.rules`, `test-static.sh` | Oui — inspection contributive sûre, présente dans le ZIP source mais non installée par RPM/DEB | Oui — actions status/diff/diff-staged/diff-check/inventory fixes, environnement fermé, Git système, confiance limitée au chemin canonique du checkout et écritures optionnelles désactivées | Non | Non, fournit la frontière exécutable que les règles par préfixe ne peuvent pas exprimer seules | KEEP |
...
| `scripts/update-published-version.py` | Python développement | Met à jour de façon bornée les références de release publiées dans les deux README et leur contrat statique | Workflow documentaire post-release, mainteneurs, `test-static.sh` | Oui — évite les exemples d’installation obsolètes après publication | Oui — SPDX/docstring, cardinalités exactes, staging complet, rollback best-effort et mode check | Non | Non, centralise la transformation sans publier ni contourner la revue | KEEP |
...
- Nombre total de fichiers tracked : **86**.
- Nombre de fichiers examinés : **86**.
- Nombre de fichiers à conserver : **86**, dont les catégories historique et

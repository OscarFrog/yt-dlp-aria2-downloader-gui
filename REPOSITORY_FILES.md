...
| `.github/workflows/packages.yml` | CI workflow | Source-only PR package CI after its own cheap identity/coherence/syntax gate: validates a Git-free archive, builds RPM/DEB, authenticates the previous immutable release and exercises package lifecycle/upgrades. | KEEP |
...
| `.github/workflows/qualification.yml` | CI workflow | Source-only PR FFmpeg generation matrix after its own cheap gate, sharing Ubuntu baseline fixtures with real-tools; the Fedora download job uses disposable local storage volumes subject to private-filesystem validation. | KEEP |
| `.github/workflows/real-tools.yml` | CI workflow | Source-only PR qualification with pinned yt-dlp matrices after its own cheap gate, one shared Ubuntu FFmpeg baseline and bounded real aria2 repetitions; separately checks scheduled current-stable yt-dlp. | KEEP |
...
| `.github/workflows/stress.yml` | CI workflow | Source-only twenty distinct signal-jitter tuples and ten runtime transaction cycles after its own cheap gate; the unchanged required check aggregates all complementary PR workflows before merge. | KEEP |
...
| `CI_AUDIT.md` | Audit report | Source-only dated CI/runtime/test timing, duplication, protection and implementation evidence, including Phase 2 benchmarks and independent Phase 3 findings; records limitations without replacing operating policy. | KEEP |
...
| `download-video-gui.sh` | Bash entrypoint | Installed RPM/DEB Zenity frontend, also used from Git/ZIP; offers two URL-compatible profiles with remembered defaults, uses the shared private allocator, passes the destination to the engine and preserves active state until shutdown is confirmed. | KEEP |
...
| `scripts/ci-validation.py` | Python validation | Source-only authenticated Actions proof verifier shared by the final PR gate, main promotion, release and evidence tooling; reuses validated immutable Git identities while freshly binding active workflows and latest successful runs/jobs to the exact virtual merge and squash tree. | KEEP |
...
| `scripts/dev-tools/ensure-shfmt.sh` | Bash tool bootstrap | Source-only formatter resolver used by formatting scripts; verifies pinned bytes, serializes cold-cache repair with an identity-checked lock and recheck, and returns the exact executable path. | KEEP |
...
| `test-static.sh` | Static validation | Source-only complete standalone static contract; source identities/inventories, syntax, versions and workflow contracts plus Python replays. Its explicit partial source-only mode lets the canonical runner schedule those replays once under the same job bound. | KEEP |
...
| `tests/ci-validation-integration.py` | Python test | Source-only standalone-static/canonical-scheduler adversarial proof, workflow graph and bounded signing-preflight tests; protect fresh workflow/run identity, complete gates and absence of redundant main/release suites. | KEEP |
| `tests/source-archive-integration.py` | Python test | Source-only real Git/ZIP fixtures, called by standalone static and the canonical scheduler; reject changed bytes, paths, modes, symlinks and commit identities before source promotion. | KEEP |
...
| `tests/lib/test-runner.sh` | Sourced Bash/Python test library | Source-only bounded child/session supervision, provisional registration before foreground commands, same-PID monitor-group transition, authenticated session identity, cancellation and timing helpers used by both runners; preserves caller options. | KEEP |
...
| `tests/progress-monitor-integration.sh` | Integration test | Source-only fast/full qualification of progress aggregation and phase transitions, including unchanged ETA and destination-copy messages while local-storage diagnostics stay out of the Zenity display. | KEEP |
| `tests/push-version-integration.py` | Python test | Standalone-static/canonical-scheduler qualification of the real Git hook and source-version preparation using private repositories; covers omissions, partial staging, remote baselines, deterministic transforms, interruption and rollback. | KEEP |
| `tests/release-docs-integration.py` | Python test | Source-only actual documentation workflow replay with private copies of an inert intra-run seed and independent simulated APIs; standalone static and the canonical scheduler qualify transforms, streaming, no-op, publication, races and handoffs. | KEEP |
| `tests/shfmt-version-handoff-integration.py` | Python test | Source-only actual shfmt workflow replay using private Git repositories and controlled upstream/container stubs; standalone static and the canonical scheduler qualify independent bumps, publication leases, no-ops and rejected mutations. | KEEP |
| `tests/shfmt-bootstrap-integration.py` | Python test | Source-only bootstrap qualification with private pinned assets; covers concurrent/stale cold misses, cache reuse, rejected symlinks, failed downloads, checksum/version failures, interruptible locking and cleanup of nested fixture sessions. Called by standalone static validation and the canonical scheduler. | KEEP |
| `tests/real-tools-integration.sh` | Real-tool qualification | Source-only real local HTTP fixtures for direct/audio/HLS/DASH, isolated no-overwrite assertions, cooperative engine timeout cleanup, optional permissive-directory simulation and explicit disposable network-share qualification. | KEEP |
| `tests/repeat-qualification.sh` | Bash qualification runner | Source-only bounded parallel repetition of independent commands with ordered logs and monitor-mode cancellation; available for explicit repeat qualifications and exercised by runner integration; ordinary CI does not wrap identical deterministic fixtures in redundant repetitions. | KEEP |
...
| `tests/run-all-signal-integration.sh` | Integration test | Source-only full-suite HUP/INT/TERM and descendant-cleanup qualification of the canonical test runner; drains subprocess diagnostics during waits, reports signal-guard timeouts and qualifies pipe-capacity backpressure. | KEEP |
| `tests/run-all.sh` | Bash validation runner | Source-only canonical doctor and fast/full scheduler; runs formatting/source assertions/ShellCheck and explicit Python suites under one static job bound, then isolated integration suites using shared process supervision and monitor-mode cancellation. | KEEP |
...
| `tests/test-runner-integration.sh` | Integration test | Source-only fast/full regression for runner supervision, real terminal cancellation, monitor-group/session transitions, real-tool engine timeout and assertion isolation, scheduling, status propagation and exact validation manifests. | KEEP |

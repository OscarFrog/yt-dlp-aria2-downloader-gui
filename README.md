...
The exact asset names below match the currently published v2.3.14 release.
...
yt-dlp-aria2-downloader-gui-2.3.14-1.fc44.noarch.rpm
SHA256SUMS
```
...
bash ./install-fedora.sh ./yt-dlp-aria2-downloader-gui-2.3.14-1.fc44.noarch.rpm
```
...
it, then install it with `sudo apt install ./yt-dlp-aria2-downloader-gui_2.3.14-1_all.deb`.
...
gh release verify v2.3.14 -R OscarFrog/yt-dlp-aria2-downloader-gui
gh release verify-asset v2.3.14 ./ARTIFACT -R OscarFrog/yt-dlp-aria2-downloader-gui
```
...
The latest published release, 2.3.14, provides an architecture-independent DEB
aligned with the same managed-runtime model as Fedora. Download:

```text
yt-dlp-aria2-downloader-gui_2.3.14-1_all.deb
SHA256SUMS
```
...
```bash
sha256sum --ignore-missing --check SHA256SUMS
sudo apt install ./yt-dlp-aria2-downloader-gui_2.3.14-1_all.deb
```
...
```text
yt-dlp-aria2-downloader-gui-2.3.14.zip
SHA256SUMS
```
...
```bash
sha256sum --ignore-missing --check SHA256SUMS
unzip yt-dlp-aria2-downloader-gui-2.3.14.zip
cd yt-dlp-aria2-downloader-gui-2.3.14

...
it, then install it with `sudo apt install ./yt-dlp-aria2-downloader-gui_2.1.34-1_all.deb`.
...
- static, mock, and hermetic real-tool integration tests, including local
  ...
  aria2 Range/no-Range/redirect/error/resume behavior, real FFmpeg progress,
...
gh release verify-asset v2.1.34 ./ARTIFACT -R OscarFrog/yt-dlp-aria2-downloader-gui
```
...
Release 2.1.34 publishes an architecture-independent DEB aligned with the same

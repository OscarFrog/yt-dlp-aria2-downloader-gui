...
# Hermetic direct-transfer qualification using real yt-dlp, aria2c, FFmpeg, and FFprobe.
...
case ${BASIC_RUNS} in
'' | *[!0-9]*) printf 'Error: ARIA2_BEHAVIOR_BASIC_RUNS must be a positive integer.\n' >&2; exit 64 ;;
*) ;;
esac
case ${RESUME_RUNS} in
...
# Resume proof: cancel only after the server has completed a substantial partial
# 206 response (large enough to exclude tiny capability probes), retain aria2's
# partial state, then prove the second run fetches fewer total media bytes than a
# complete redownload while still producing a valid final file. Do not assume a
# particular split starts at byte zero; aria2 is free to schedule ranges.
